...
{{ formataPallet($pallet->numero_pal) }} {{ $pallet->codigo_pro }}<?php for($i=strlen($pallet->codigo_pro); $i<8; $i++) echo " "; ?> {{ $pallet->descri_pro }}     <?php for($i=strlen($pallet->descri_pro); $i<48; $i++) echo " "; ?> {{ formataNumero($pallet->saldo_pal) }}<?php for($i=strlen(formataNumero($pallet->saldo_pal)); $i<9; $i++) echo " "; ?>{{ formataNumero($pallet->pesol_pal) }}<?php for($i=strlen(formataNumero($pallet->pesol_pal)); $i<10; $i++) echo " "; ?>{{ formataNumero($pallet->pesob_pal) }}<?php for($i=strlen(formataNumero($pallet->pesob_pal)); $i<=10; $i++) echo " "; ?>{{ mysqlToBr($pallet->dta_ent) }} {{ mysqlToBr($vencto) }} {{ $pallet->nfe_ent }}<?php for($i=strlen($pallet->codigo_pro); $i<6; $i++) echo " "; ?>   {{ $pallet->obs1_pal }}{{"\r\n"}}
<?php 
  $totalPallets++;
